...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Truy vấn để lấy phần trăm giảm giá từ bảng khuyenmai
    $km_sql = "SELECT phantramgiam FROM khuyenmai WHERE id = ?";
    $km_stmt = $conn->prepare($km_sql);
    $km_stmt->bind_param("i", $khuyenmai_id);
    $km_stmt->execute();
    $km_result = $km_stmt->get_result();
    $khuyenmai = ($km_row = $km_result->fetch_assoc()) ? $km_row['phantramgiam'] : 0;
